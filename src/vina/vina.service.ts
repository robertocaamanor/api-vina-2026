...
import { ArtistaDia, ParrillaFestival } from './vina.interfaces';

@Injectable()
export class VinaService {
  private readonly festivalData: ParrillaFestival = {
  ...
  getParrilla(): ParrillaFestival {
    return this.festivalData;
  }

  getProgramacionPorDia(diaBuscado: string): ArtistaDia {
    const resultado = this.festivalData.programacion.find(
      (p) => p.dia.toLowerCase() === diaBuscado.toLowerCase(),
    );

    if (!resultado) {
      throw new NotFoundException(
        `No se encontró programación para el día: ${diaBuscado}`,
      );
    }
